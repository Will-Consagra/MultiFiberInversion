...
Psi_hat_21_bv1 = psi_map[2][:, 0, 0, :]
Psi_hat_21_bv3 = psi_map[2][:, 1, 0, :]

Psi_hat_22_bv1 = psi_map[2][:, 0, 1, :]
Psi_hat_22_bv3 = psi_map[2][:, 1, 1, :]

Signals_21_hat = torch.cat((torch.from_numpy(Psi_hat_21_bv1).float().unsqueeze(1), 
						torch.from_numpy(Psi_hat_21_bv3).float().unsqueeze(1)), dim=1)
Signals_22_hat = torch.cat((torch.from_numpy(Psi_hat_22_bv1).float().unsqueeze(1), 
								torch.from_numpy(Psi_hat_22_bv3).float().unsqueeze(1)), dim=1)
Signals_2 = torch.cat((Signals_21_hat, Signals_22_hat), dim=0)

with open(os.path.join(TRAINDIR, "xi_map.pkl"), "rb") as pklfile:
	xi_map = pickle.load(pklfile)

theta_2_stacked = torch.from_numpy(xi_map[2]).float()
theta_2 = torch.cat((theta_2_stacked[:,0,:], theta_2_stacked[:,1,:]), dim=0)

## inverse model 
input_channels = len(bvals)
p = 5
input_size = Signals_2.shape[-1]
...
dataset_abc = torch.utils.data.TensorDataset(theta_2, Signals_2)
...
for epoch in range(num_epochs):
	...
	for batch in dataloader_train:
		...
		#pbar.update(1)
		if not iter_ % 1000:
			torch.save(inverse_model.state_dict(), os.path.join(MODELDIR, "shape_inverse_2fib_%s_%s.pth"%(suffix, iter_)))
